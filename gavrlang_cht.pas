...
Const
  ...
  as:Array[1..nas] Of WideString=(
    ...
    {46} 'relative jump address (label) in the range +/- 2k, 相對跳轉地址（標籤）在+/- 2k範圍內, ',
    {47} 'register in the range from R16 to R31, 寄存器 R16 - R31 ',
    {48} 'double register R24, R26, R28 or R30, 雙寄存器 R24,R26,R28 或 R30 ',
    {49} 'lower port value between 0 and 31, 端口值 0 - 31 ',
    {50} 'register in the range from R16 to R23, 寄存器 R16 - R23 ',
    {51} 'even register (R0, R2 ... R30), 偶數寄存器（R0, R2 ... R30) ',
    {52} 'port value in the range from 0 to 63, 端口值 0 - 63 ',
    {53} 'X/Y/Z or X+/Y+/Z+ or -X/-Y/-Z ',
    {54} 'Y+distance or Z+distance, range 0..63, Y+偏移量 或 Z+偏移量, 範圍 0..63 ',
    {55} '16-bit SRAM address, 16位SRAM地址 ',
    {56} 'Constant in the range 0..63, 常量範圍 0..63 ',
    {57} 'Constant in the range 0..255, 常量範圍 0..255 ',
    {58} 'parameter, 參數 ',
    {59} 'Internal compiler error! Please report to gavrasm@avr-asm-tutorial.net! 編譯器錯誤! 請回報給作者 ',
    ...
    {63} '.CSEG     : compiles into the code segment, 編譯成代碼段 ',
    {64} '.DB x,y,z : inserts Bytes, chars or strings (.CSEG, ESEG), 插入位元組, 字符, 字串（.CSEG,ESEG） ',
    {65} '.DEF x=y  : symbol name x is attached to register y, 符號名稱x 附加到寄存器y ',
    {66} '.DEVICE x : check the code for the AVR type x, 檢查 AVR 類型 x 的代碼 ',
    {67} '.DSEG     : data segment, only labels and .BYTE directives, 數據段,只有標籤和 .BYTE 指令 ',
    {68} '.DW x,y,z : insert words (.CSEG, .ESEG), 插入字節（.CSEG,.ESEG） ',
    {69} '.ELIF x   : .ELSE with condition x, ',
    {70} '.ELSE     : alternative code, if .IF-condition was false, 如果.IF條件為false 替代的源碼 ',
    {71} '.ENDIF    : closes .IF resp. .ELSE or .ELIF, 關閉.IF .ELSE或.ELIF ',
    ...
    {82} '.LIST     : switches list output on, 開啟列表功能 ',
    ...
    {87} '.NOLIST   : switches list output off, 關閉列表功能 ',
    {88} '.ORG x    : sets the CSEG-/ESEG-/DSEG-counter to value x, 將 CSEG-/ESEG-/DSEG- 賦值為 x ',
    {89} '.SET x=y  : sets the variable symbol x to the value y, 將變量符號 x 賦值為 y ',
    {90} '.SETGLOBAL x,y,z: globalize the local symbols x, y and z 將本地符號 x,y,z 設為全局變數, ',
    {91} '.UNDEF x  : undefines the symbol x, 未定義符號 x ',
  ...
  asw:Array[1..nasw] Of WideString=(
    ...
    '009: Include defs not necessary, using internal values! 不必要的 defs, 使用內部值!',
    '010: Instruction set unclear, no documentation! 指令集不清楚, 無文件!',
    '011: C-style instructions in file, lines ignored! 文件中的C格式的指令,忽略該行!'
	);
  ...
  ase:Array[1..nase] Of WideString=(
    ...
    '045: Missing ORG address, no parameters! 缺少ORG的地址, 沒有參數!',
    '046: Origin address (%1) points backwards in %2! 原始地址 (%1) 在%2 中向後點',
    '047: Undefined ORG constant! 未定義ORG常數!',
    '048: Too many parameters on ORG line, only address! ORG線上的參數太多, 只需地址!',
    ...
    '051: Expected device name, no name found! 找不到MCU型號!',
    '052: Device already defined! MCU型號已經定義!',
    '053: Unknown device, run gavrasm -T for a list of supported devices! MCU型號未知, 運行 gavrasm -T 以獲得支持的型號列表!',
    '054: Too many parameters, expecting device name only! 參數太多, 只要MCU型號!',
    ...
    '058: Too many parameters, expected a macro name only! 參數太多, 只要一個巨集名!',
    '059: Closing macro without one open or macro empty! 關閉巨集沒有打開或巨集空!',
    '060: .IF condition missing! 缺少.IF條件指示!',
    ...
    '085: Error in logical expression of the EXIT-directive! 邏輯表達式 EXIT-directive 出錯!',
    '086: Break condition is true, assembling stopped! 停止條件出現, 編譯停止!',
    '087: Illegal literal constant (%1)! 非法文字常數 (%1) !',
    '088: Illegal string constant (%1)! 非法字符串常量 (%1)!',
    '089: String (%1) not starting and ending with "!" 字串開始結尾沒有“!”',
    '090: Unexpected parameter or trash on end of line! 行末尾的意外參數或垃圾字元!',
    '091: Missing or unknown parameter(s)! 丟失或未知參數!',
    '092: Unknown device name (%1)! MCU型號未知 (%1)!',
    ...
    '097: Unrecognized include file, use .DEVICE instead! 無法識別的包含文件, 使用 .DEVICE 代替!',
    '098: Device doesn''t support Auto-Inc/Dec statement! MCU型號不支持 Auto-Inc/Dec 指令!',
    '099: IF statement without ENDIF! 邏輯判斷 IF 語句沒有對應的 ENDIF!',
    '100: Multiple DEVICE definition! 多重 DEVICE 定義',
